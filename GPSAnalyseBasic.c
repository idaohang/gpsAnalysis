...
struct tm *timeStructFromString(char *tstring, struct tm *tstruct);
...
double calculate_tot_dist(struct node *lh){
    ...
    strcpy(finishTimeStr, curr->timeString);
    	...
    	tm1 = * timeStructFromString(startTimeStr, &tm1);
    	tm2 = * timeStructFromString(finishTimeStr, &tm2);

    	printf("Elapsed Time: %d sec", timeDiff(&tm1, &tm2));

    	averagePace = timeDiff(&tm1, &tm2)/pathLen*1000.0/60.0;
...
// A function to populate a tm time structure from an GPX time string.
// e.g. "2013-09-12T15:59:18Z"

struct tm *timeStructFromString(char *tstring, struct tm *tstruct){

	strptime(tstring, "%Y-%m-%dT%H:%M:%S", tstruct);

	return tstruct;
}

// A function to return the time difference in seconds between
// two tm time structures.
// It assumes the two times are from the same day.

int timeDiff(struct tm *t1, struct tm *t2){
	int ret = 0, s1 = 0, s2 = 0;
	s1 = t1->tm_hour * 3600 + t1->tm_min * 60 + t1->tm_sec;
	s2 = t2->tm_hour * 3600 + t2->tm_min * 60 + t2->tm_sec;
	ret = s2 - s1;
	return ret;
}
